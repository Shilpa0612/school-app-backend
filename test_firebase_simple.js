#!/usr/bin/env node

/**
 * Simple Firebase Test without dotenv
 */

console.log('🔥 Testing Firebase Notifications...\n');

// Test 1: Check if Firebase Admin SDK is installed
console.log('1. Checking Firebase Admin SDK...');
try {
    const admin = require('firebase-admin');
    console.log('   ✅ Firebase Admin SDK is installed');
} catch (error) {
    console.log('   ❌ Firebase Admin SDK not installed');
    console.log('   📦 Run: npm install firebase-admin');
    process.exit(1);
}

// Test 2: Test Firebase initialization with hardcoded values
console.log('\n2. Testing Firebase Initialization...');
try {
    const admin = require('firebase-admin');

    // Use the values from your .env file
    const serviceAccount = {
        type: "service_account",
        project_id: "school-app-notifications-c86ab",
        private_key_id: "8d9941730f785325d49cc201ead5f94cc04e458f",
        private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n",
        client_email: "firebase-adminsdk-fbsvc@school-app-notifications-c86ab.iam.gserviceaccount.com",
        client_id: "118128845168150254613",
        auth_uri: "https://accounts.google.com/o/oauth2/auth",
        token_uri: "https://oauth2.googleapis.com/token",
        auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
        client_x509_cert_url: "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-fbsvc%40school-app-notifications-c86ab.iam.gserviceaccount.com"
    };

    if (!admin.apps.length) {
        admin.initializeApp({
            credential: admin.credential.cert(serviceAccount),
            projectId: "school-app-notifications-c86ab"
        });
    }

    console.log('   ✅ Firebase initialized successfully');
    console.log('   📱 Project ID: school-app-notifications-c86ab');
    console.log('   📧 Service Account: firebase-adminsdk-fbsvc@school-app-notifications-c86ab.iam.gserviceaccount.com');

} catch (error) {
    console.log('   ❌ Firebase initialization failed');
    console.log('   🔍 Error:', error.message);
    process.exit(1);
}

// Test 3: Test sending a push notification
console.log('\n3. Testing Push Notification Sending...');
try {
    const admin = require('firebase-admin');

    // Test message
    const testMessage = {
        token: 'test_token_12345', // This will fail, but we can test the service
        notification: {
            title: 'Test Notification',
            body: 'This is a test push notification from your school app!'
        },
        data: {
            type: 'test',
            id: 'test-' + Date.now()
        }
    };

    // This will fail because the token is fake, but it tests the service
    admin.messaging().send(testMessage)
        .then((response) => {
            console.log('   ✅ Push notification sent successfully:', response);
        })
        .catch((error) => {
            if (error.code === 'messaging/invalid-registration-token') {
                console.log('   ✅ Firebase service is working (expected error for fake token)');
                console.log('   📝 Error:', error.message);
            } else {
                console.log('   ❌ Unexpected error:', error.message);
            }
        });

} catch (error) {
    console.log('   ❌ Error testing push notification:', error.message);
}

console.log('\n🎉 Firebase Test Complete!');
console.log('\n📋 Next Steps:');
console.log('1. Start your server: npm start');
console.log('2. Register a real device token');
console.log('3. Send test notifications to real devices');

console.log('\n🔗 Test Commands:');
console.log('   Start server: npm start');
console.log('   Test with real device: node test_push_notifications.js');
